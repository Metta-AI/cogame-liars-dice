...
# Substitute: liars-dice, coworld-liars-dice, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-liars-dice:ci)
#   SMOKE_SLUG                 game slug                        (liars-dice)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/liars-dice)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/liars-dice-player)
...
#   LIARS_DICE_JEV, OPENROUTER_API_KEY
...
image="${1:-${SMOKE_IMAGE:-coworld-liars-dice:ci}}"
slug="${SMOKE_SLUG:-liars-dice}"
...
game_env=()
if [ "${LIARS_DICE_JEV:-}" = "1" ]; then
  game_env+=(-e LIARS_DICE_JEV=1)
...
elif [ "${LIARS_DICE_JEV:-}" = "1" ]; then
  echo "LIARS_DICE_JEV enabled: the System One path will be exercised"
else
